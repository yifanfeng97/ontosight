"""Storage engines for graph and hypergraph data with efficient indexing and neighborhood queries."""
...
class GraphStorage(BaseStorage):
    ...
    def get_sample(self, center_ids: Optional[List[str]] = None, hops: int = 2) -> Dict[str, Any]:
        """Get a subgraph around given center nodes (or random if not provided)."""
        if not center_ids:
        ...
        visited_nodes = set(center_ids)
        visited_edges = set()
        current_layer = set(center_ids)
...
class HypergraphStorage(BaseStorage):
    ...
    def get_sample(self, center_ids: Optional[List[str]] = None, hops: int = 2) -> Dict[str, Any]:
        """Get sub-hypergraph around given nodes via hyperedge neighborhoods."""
        if not center_ids:
            # Select random node with degree > 0 (not isolated)
